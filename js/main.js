...
$__System.register('b', ['7', '9', 'a'], function (_export) {
	...
	var _, $, lg, $menuButton, $menuDrawer, $menuLink, $header, $row, $rowStoreAndRewards, $rowGroup, menuState, headerHeight, id, child, debounce_shift_menu_display_links;

	//let debounce_shift_menu_display_links = _.debounce(shift_menu_display_links, 10);
	// ----------------- MENU SHOW AND HIDE -----------------
	function show_menu() {
	...
	// ----------------- MENU SHIFT -----------------
	function shift_menu() {
		// Reset all rows, groups, and row headers
		$rowGroup.removeClass('active');
		$('.row-header').removeClass('row-header current');
		// Keep Store and Rewards in view for top-level menu, Account menu, and Gifts menu
		if (child !== 1 && child !== 202 && child !== 203) {
			$rowStoreAndRewards.css({ 'display': 'none' });
		} else {
			$rowStoreAndRewards.css({ 'display': 'flex' });
		}
		// Set parent row as a header row
		var $newHeader = $('.row[data-id="' + id + '"]');
		$newHeader.addClass('row-header current');
		// Go up a few parents to check additional parent rows to assign as row headers
		var pg1 = $newHeader.parent().data('group');
		if (pg1) {
			var $pr1 = $('.row[data-child="' + pg1 + '"]');
			$pr1.addClass('row-header').css({ 'display': 'block' });
			var pg2 = $pr1.parent().data('group');
			if (pg2) {
				var $pr2 = $('.row[data-child="' + pg2 + '"]');
				$pr2.addClass('row-header').css({ 'display': 'block' });
				var pg3 = $pr2.parent().data('group');
				if (pg3) {
					var $pr3 = $('.row[data-child="' + pg3 + '"]');
					$pr3.addClass('row-header').css({ 'display': 'block' });
					var pg4 = $pr3.parent().data('group');
					if (pg4) {
						var $pr4 = $('.row[data-child="' + pg4 + '"]');
						$pr4.addClass('row-header').css({ 'display': 'block' });
						var pg5 = $pr4.parent().data('group');
						if (pg5) {
							var $pr5 = $('.row[data-child="' + pg5 + '"]');
							$pr5.addClass('row-header').css({ 'display': 'block' });
							var pg6 = $pr5.parent().data('group');
							if (pg6) {
								var $pr6 = $('.row[data-child="' + pg6 + '"]');
								$pr6.addClass('row-header').css({ 'display': 'block' });
								var pg7 = $pr6.parent().data('group');
								if (pg7) {
									var $pr7 = $('.row[data-child="' + pg7 + '"]');
									$pr7.addClass('row-header').css({ 'display': 'block' });
									var pg8 = $pr7.parent().data('group');
									if (pg8) {
										var $pr8 = $('.row[data-child="' + pg8 + '"]');
										$pr8.addClass('row-header').css({ 'display': 'block' });
									}
								}
							}
						}
		...
		debounce_shift_menu_display_links();
	}
	function shift_menu_display_links() {
		// Display new group rows
		var $newGroup = $('.group[data-group="' + child + '"]');
		var $newRows = $newGroup.find('.row');
	...
	return {
		...
		execute: function () {
			...
			// FUNCTIONS
			// ----------------- DEBOUNCES -----------------
			debounce_shift_menu_display_links = _.debounce(shift_menu_display_links, 50);
